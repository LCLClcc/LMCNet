...
class FewShotDataset_test(Dataset):
    ...
    def __init__(self,
                 ...
                 epoch_size=2000,  # number of tasks per eoocha
    ...
    def _creatExamplesTensorData(self, examples):
        """
        Creats the examples image label tensor tiered_dataloader.
        ...
        return images, labels, or_images

    def __getitem__(self, index):
        Tnovel = self.Epoch_Tnovel[index]
        Exemplars = self.Epoch_Exemplar[index]
        Xt, Yt, Xot = self._creatExamplesTensorData(Exemplars)
        Xe, Ye, Xet = self._creatExamplesTensorData(Tnovel)
        return Xt, Yt, Xe, Ye,
