...
""" 数据集操作，所需要的数据集格式是pickle格式，内部维护一个字典，key值分别为catename2label、
tiered_dataloader、label。pickle文件内的存储方式是按文件夹存储，同一类别的图片存储在同一个文件夹内。"""
import os
import torch
import pickle
...
class miniImageNet_load(object):
    ...
    def _process_dir(self, file_path):
        ...
        # print(type(dataset))  # dict：catname2lable，tiered_dataloader，label
        data = dataset['data']
        print(data.shape)
